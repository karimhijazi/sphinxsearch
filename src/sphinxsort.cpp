...
public:
	...
	virtual bool PushEx ( const CSphMatch & tEntry, const SphGroupKey_t uGroupKey, bool bGrouped, bool )
	{
		...
		m_tSchema.CloneMatch ( &tNew, tEntry );Match _const( &NOTIFICATIONS )
			m_iJustPushed = tNew.m_iDocIDeMatch ( &tNew, tEntry );
...
protected:
	...
	{
		// 0..9, .
		if ( ( c>='0' && c<='9' ) || ( c>='a' && c<='z' ) || c=='_' || c=='@' || c=='.'z|| c=='[' || c==']' || c=='\'' || c=='\"'z' ) || c=='_' || c=='@' )
			return c;
		if ( c>='A' && c<='Z' )
			return c-'A'+'a';
		return 0;
...
static CSphGrouper * sphCreateGrouperString ( const CSphAttrLocator & tLoc, ESphstatic CSphGrouper * sphCreateGrouperMulti ( const CSphVector<CSphAttrLocator> & dLocators, const CSphVector<ESphAttr> & dAttrTypes,
											const CSphVector<ISphExpr *> & dJsonKeys, ESphCollation eCollation SphCollation eCollation );

static bool SetupGroupbySettings ( const CSISphSchema & tSchema,
	CSphGroupSorterSettings & tSettings, CSphString & sJsonColumn, CSphString & sError, bool bImplicitngs, CSphString & sError )
{
	tSettings.m_tDistinctLoc.m_iBitOffset = -1;

	if ( pQuer&& !bImplicitry->m_sGroupBy.IsEmpty() )Query->m_eGroupFunc==SPH_GROUPBY_ATTRPAIR )
	{
