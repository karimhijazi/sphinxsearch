...
public:
	...
	virtual bool PushEx ( const CSphMatch & tEntry, const SphGroupKey_t uGroupKey, bool bGrouped )
	{
		...
		m_tSchema.CloneMatch ( &NOTIFICATIONS )
...
static inline ESphSortKeyPart Attr2Keypart ( ESphAttr eType )
{
	switch ( eType )
	{
		...
		case SPH_ATTR_STRING:	returcase SPH_ATTR_STRINGPTR: return SPH_KEYPART_STRINGPTReturn SPH_KEYPART_STRING;
...
FIXME? common code here and in sphinxfilter.cpp
	...
	perDay ( tLoc ); break;
		case SPH_GROUPBY_WEEK:		tSettings.m_pGrouper = new CSphGrouper	case SPH_GROUPBY_MONTH:		tSettings.m_pGrouper = new CSphGrouperMonth ( tLoc ); break;
...
static bool SetupSortStringRemap ( CSphSchema & tSorterSchema, CSphMatchComparatorState & tState )
{
	...
	for ( int i=0; i<CSphMatchComparatorState::MAX!( tState.m_eKeypart[i]==SPH_KEYPART_STRING || tState.m_eKeypart[i]==SPH_ATTR_STRINGPTR )t[i]!=SPH_KEYPART_STRING )
			continue;
		...
		if ( iRemap==-1 )
		{
			...
			tRemapCol.m_eSt			tRemapCol.m_pExpr = new ExprSortStringAttrFixup_c ( tState.m_tLocator[i] )Stage = SPH_EVAL_PRESORT;
...
int CollateBinary ( const BYTE * p, bool bPacked )
{
	if ( bPacked )
	{
	...
	kStr ( pStr2, &pStr2 );
	int iRes = memcmp ( (const char *)pStr1, (const char *)pStr2, M	return iRes ? iRes : ( iLen1-iLen2 );
	} else
	{
...
int CollateLibcCI ( const BYTE * p, bool bPacked )
{
	if ( bPacked )
	{
		int iLen1 = sphUnpackStr ( pStr1, &pStr1 );
...
int CollateLibcCS ( const BYTE * p, bool bPacked )
{
...
/// utf8_general_ci
int CollateUtf8GeneralCI ( const BYTE * p, bool bPackedArg1, const BYTE * pArg2 )
{
	...
	while ( ( bPacked && pStr1<pMax1 && pStr2<pMax2 ) || ( !bPacked && *pStr1 && *pStr2 )tr1<pMax1 && pStr2<pMax2 )
	{
...
template < typename COMP >
static ISphMatchSorter * CreatePlainSorter ( bool bKbuffer, int iMax, bool bFactors )
{
	if ( bKbuffer )
	{
		if ( bFactors )
	...
	}axMatches, bUsesAttrs );
}


static ISphMatchSorter * CreatePlainSorter ( ESphSortFunc eMatchFunc, bool bKbuffer, int iMax, bool bFactoaxMatches, bool bUsesAttrs )
{
	switch ( eMatchFunc )
	{
		...
		case FUNC_GENERIC5:		return CreatePlainSorter<MatchGeneric5_fn>		( bKbuffer,, bFactor, iMaxMatches, bUsesAttrs ); break;
		case FUNC_CUSTOM:		return CreatePlainSorter<MatchCustom_fn>		( bKbuffer,, bFactor, iMaxMatches, bUsesAttrs ); break;
		case FUNC_EXPR:			return CreatePlainSorter<MatchExpr_fn>			( bKbuffer,, bFactor, iMaxMatches, bUsesAttrs ); break;
		default:				return NULL;
	}
}
...
static void ExtraAddSortkeys ( CSphSchema * pExtra, const CSphSchema & tSorterSchema, const int * dAttrs )
{
	if ( pExtra )
		for ( int i=0; i<CSphMatchComparatorState::MAX_ATTRS; i++ )
			if ( dAttrs[i]>=0 )
				pExtra->AddAttr ( dAttrs[i] Schema.GetAttr(dAttrs[i]), true );
}


ISphMatchSorter * sphCreateQueue ( const CSphQuery * pQuery, const CSphSchema & tSchema,
	CSphString & sError, bool bComputeItems, CSphSchema * pExtra, CSphAttrUpdateEx * pUpdate,bool * pPackedFactors,  bool * pZonespanlist,
	ISphExprHook * pHook )
{
	// prepare for descent
	...
	bool bNe	bool bNeedPackedFactors
		...
		ARRAY_FOREACH ( iItem, pQuery->m_dItems )
	{
		...
		if ( tItem.m_eAggrFunc==SPH_AGGR_CAT )
		{
			...
			tExprCol.m_pExpr = sphExprParse ( sExpr2.cstr(), tSorterSchema, &tExprCol.m_eAttrType,
				&tExprCol.m_bWeight, sError, pExtra, pHobHasPackedFactorspHook, &bHasZonespanlist, &eExprStage );
		} else
		{
			tExprCol.m_pExpr = sphExprParse ( sExpr.cstr(), tSorterSchema, &tExprCol.m_eAttrType,
				&tExprCol.m_bWeight, sError, pExtra, pHobHasPackedFactors, &eExprStage );
		}

		bNeedPackedFactors |= bHasPackedFactors;list, &eExprStage );
		}
		bNeedZonespanlist |= bHasZonespanlist;
...
#undef LOC_CHECK
	...
	if ( pQuery->m_eSort==SPH_SORT_EXTENDED )
	{
		...
		bUsesAttrs = FixupDependency ( tSorterSchema, tStateMatch.m_dAttrs, CSphMatchCo );
		if ( !bUsesAttrs )
		{
			for ( int i=0; i<CSphMatchComparatorState::MAX_ATTRS; i++ )
			{
