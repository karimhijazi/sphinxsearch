...
public:
		...
		: CSphMatchQueueTraits ( iSize, bUsesAttrs )
	{
		if_const ( NOTIFICATIONS )
			m_dJustPopped.Reserve(1Locator ( m_tMvaLocator );
	}
...
public:
		...
		, m_bFinalized ( false )
	{
		...
		m_iSize /= COEFFocator ( m_tMvaLocator );
	}
...
{
public:
	uint64_t Hash ( con, uint64_t uPrev=SPH_FNV64_SEED ) const
	{
		assert ( pStr && iLen );
...
public:
	/// ctor
	CSphKBuffer/// ctor
	CSphKBufferMVAGroupSorter ( const ISphMatchComparator * pComp, const CSphQuery * pQuery, const CSphGroupSort // FIXME! make k configurable
	...
	virtual bool PushEx ( const CSphMatch & tEntry, const SphGroupKey_t uGroupKey, bool bGrouped, bool, SphAttr_t * pAttr=NULL )
	{
		...
		if ( ppMatch )if we're full, let's cut off some worst groups
		if ( m_iUsed==m_iSize )
		...
		m_tSchema.CloneMatch ( &tNew, tEntry );Match _const( &NOTIFICATIONS )
			m_iJustPushed = tNew.m_uDocIDeMatch ( &tNew, tEntry );

		if ( !bGrouped )
		{
			tNew.SetAttr ( m_tLocGroupby, uGroupKey );
			tNew.SetAttr (_const m_tLocCount, 1 );
			if ( DISTINCT )
				tNew.SetAttr
			// set @groupbystr value if available
			if ( pAttr && m_tLocGroupbyStr.m_bDynamic )
				tNew.SetAttr ( m_tLocGroupbyStr, *pAttrAttr ( m_tLocDistinct, 0 );
		} else
		{
			ARRAY_FOREACH ( i, m_dAggregates )
		...
		m_hGroup2Match.Add ( &tNew, uGroureturn true;
	}
...
rue );
	...
	void SetGroupState ( const CSphMatchComparatorState & tState )
	{
		...
		// check whether _constwe sort by distinct
	...
	void _constCountDistinct ()
	{
		if ( DISTINCT )
		{
			...
			for ( int iCount = m_tUniq.CountStart ( &uGroup ); iCount; iCount = m_tUniq.CountNext ( &uGroup ) )
			{
				...
				if ( ppMatch )
					(*ppMatch)->SetAttr ( m_tLocDistinct, /// cut worst N groups off the buffer tail
...
if_const ( NOTIFICATIONS )
		...
		// cleanup un_constused distinct stuff
		if ( DISTINCT )
		{
			// build kill-list
			CSphVector<SphGroupKey_t> dRemom_iUsed-iBound );
			ARRAY_FOREACH ( i, dRemove )
				dRemove[i] = m_pData[iBounmove[i] = m_pData[m_iUsed+i].GetAttr ( m_tLocGroupby );

			// sort and compact
			if ( !m_bSortByDistinct )
				m_tUniq.Sort ();
			m_tUniqm_iUsed-iBound );
		}
...
#define CHECKINTEGRITY()
	...
	virtual bool PushEx ( const CSphMatch & tEntry, const SphGroupKey_t uGroupKey, bool bGrouped, bool bNewSet )
	{
		...
		if_const ( DISTINCT )
		{
			...
			if ( bGrouped )
				iCount = (int)tEntry.GetAttr ( m_tSettings.m_tLocDistinct );
			m_tUniq.Add ( SphGroupedValue_t ( uGroupKey, tEntry.GetAttr ( m_tSettings.m_tDistinctLoc ), iCount ) ); // OPTIMIZE! use si		CHECKINTEGRITY();mpler locator here?
		}
		...
		m_dGroupsLen [ iNew ] = 1;Match _const( &NOTIFICATIONS )
			m_iJustPushed = tNew.m_uDocIDeMatch ( &tNew, tEntry );
		...
		if ( !bGrouped )
		{
			...
			tNew.SetAttr (_const m_tLocCount, 1 );
			if ( DISTINCT )
				tNew.SetAttr ( m_tLocDistincm_uLastGroupKey = uGroupKey;tinct, 0 );
		} else
		{
			ARRAY_FOREACH ( i, m_dAggregates )
				m_dAggregates[i]->Ungroup ( &tNew );
		...
		m_hGroup2Match.Add ( &tNew, uGrouCHECKINTEGRITY();
		return true;
	}
	...
	void SetGroupState ( const CSphMatchComparatorState & tState )
	{
		...
		if ( DISTINCT && m_tDistinctLoc.m_iBitOffset>=0 )
			for ( int i=0; i<CSphMatchComparatorState::MAX_ATTRS; i++ )
				if ( m_tGroupSorter.m_tLocator[i].m_iBitOffset==m_tDistin	{
					m_bSortByDistinct = true;
					break;
				}
	}
	...
	void _constCountDistinct ()
	{
		if ( DISTINCT )
		{
			...
			for ( int iCount = m_tUniq.CountStart ( &uGroup ); iCount; iCount = m_tUniq.CountNext ( &uGroup ) )
			{
				CSphMatch ** ppMatch = m_hGroup2Match(uGroup);
				if ( ppMatch )
					(*ppMatch)->SetAttr ( m_tLocDistinct, void CutWorstSubgroups ( int iBound )
	{
#ifndef NDEBUG
		++m_iruns;
#endif
		...
		// rehash
		m_hGroup2Match.Reset ();
		for ( int i=0; i<iHeadBoun	for ( int i=0; i<m_iUsed; i++ )
			m_hGroup2Match.Add ( m_pData+i, m_pData[i].GetAt
...
template < typename , bool NOTIFICATIONS >
class CSphKBufferMVAGroupSorter : public CSphKBufferGroupSorter < COMPGROUP, DISTINCT, NOTIFICATIONSer < COMPGROUP, DISTINCT >
{
protected:
	const DWORD *		m_pMva;		///< pointer to MVA poobool				m_bArenaProhibit;ool for incoming matches
...
public:
	...
	CSphKBufferMVAGroupSorter ( const ISphMatchComparator * pComp, const CSphQuery * pQuery, const CSphGroupSorterSettings & tSettings )
		: CSphKBufferGroupSort, NOTIFICATIONSer < COMPGROUP, DISTINCT > ( pComp, pQuery, tSettings )
		,ArenaProhibit ( falseings )
		, m_pMva ( NULL )
	...
	void SetMVAP, bool bArenaProhibit )
	{
		m_pMva = pMva;
		...
		phMatch & tEntry )
	{
		...
		const DWORD * pValues = tEntry.GetAttrMVA ( thi, m_bArenaProhibits->m_tMvaLocator, m_pMva ); // (this pointer is for gcc; it doesn't work otherwise)
		if ( !pValues )
			return false;
		...
		if ( m_bMva64 )
		{
			...
			for ( ;iValues>0; iValues-=2, pValues+=2 )
			{
				...
				bRes |= this->PushEx ( shEx ( tEntry, uGroupkey, false );
			}
	...
	virtual bool PushGrouped ( , bool bNewSetconst CSphMatch & tEntry )
	{
		// re-group it based on the group key
		// (first 'this' is for icc; second 'this' is for gcc)
		return this->PushEx ( tEntry, tEntry.GetAttr ( thi, bNewSet );
	}
};


/// match sorter with k-buffering and group-by for JSON arraysng and group-by for MVAs
template < typename , bool NOTIFICATIONS >
class CSphKBufferJsonGroupSorter : public CSphKBufferGroupSorter < COMPGROUP, DISTINCT, NOTIFICATIONSer < Cublic:
	/// ctor
	CSphKBufferJson/// ctor
	CSphKBufferMVAGroupSorter ( const ISphMatchComparator * pComp, const CSphQuery * pQuery, const CSphGroupSorterSettings & tSettings )
		: CSphKBufferGroupSort, NOTIFICATIONSer < COMPGROUP, DISTINCT > ( pCom{tor ( m_tMvaLocator );
	}

	/// check if this sorter does groupby
	v constirtual bool Itrue;
	}
	...
	virtual bool Push ( const CSphMatch & tMatch )
	{
		...
		bRes |= this->PushEx ( tMatch, uGroupkey, false, false, &iValue );upkey, false );
			}
		}
		return bRes;
	}
...
/// implicit group-by sorterng and group-by for MVAs
template < typename , bool NOTIFICATIONS >
...
public:
	...
	{
		return trueocator ( m_tMvaLocator );
	}
...
protected:
	...
	bool PushEx ( const CSphMatch & tEntry, bool bGrouped )
	{
		...
		if_const ( DISTINCT )
		{
			int iCount = 1;
			if ( bGrouped )
				iCount = (int)tEntry.GetAttr ( m_tSettings.m_tLocDistidUniq.Add ( SphUngroupedValue_t (( uGroupKey, tEntry.GetAttr ( m_tSettings.m_tDistinctLoc ), iCount ) ); // OPTIMIZE! use simpler locator here?
		}

		// it's a dupe anyway, so we shouldn't update totm_bDataInitialized )
			return false;
...
struct MatchRelevanceLt_fn : public ISphMatchComparator
{
	...
	static bool IsLess ( const CSphMatch & a, const CSphMatch & b, const CSphMatchComparatorState & )
	{
		if ( a.m_iWeight!=b.m_iWeight )
			return a.m_iWeight < b.muDocID > b.m_ueturn a.miDocID > b.m_iDocID;
	};
};
...
struct MatchAttrLt_fn : public ISphMatchComparator
{
	...
	static inline bool IsLess ( const CSphMatch & a, const CSphMatch & b, const CSphMatchComparatorState & t )
	{
		...
		if ( a.m_iWeight!=b.m_iWeight )
			return a.m_iWeight < b.muDocID > b.m_ueturn a.m
};
...
struct MatchTimeSegments_fn : public ISphMatchComparator
{
	...
	static inline bool IsLess ( const CSphMatch & a, const CSphMatch & b, const CSphMatchComparatorState & t )
	{
		...
		if ( aa!=bb )
			retuuDocID > b.m_ueturn a.miDocID > b.m_iDocID;
	};
...
#define SPH_TEST_KEYPART(_idx) \
	...
	{ \
		case SPH_KEYPART_IuDocID, b.m_uPAIR ( a.m_iDocID, b.m_iDocID, _idx ); break; \
...
struct MatchGeneric2_fn : public ISphMatchComparator
{
	...
	{
		SPH_TEST_KEYPART(0);
		SPH_TEa.m_uDocID>b.m_uDocID;
	}
};


struct MatchGeneric3orter
struct MatchAttrLt_fn : public ISphMatchComparator
{
	virtual bool VirtualIsLess ( const CSphMatch & a, const CSphMatch & b, const CSphMatchComparatorState & t ) const
	{
	...
	static inline bool IsLess ( const CSphMatch & a, const CSphMatch & b, const CSphMatchComparatorState & t )
	{
		SPH_TEST_KEYPART(0);
		SPH_TEST_KEYPART(1);
	return a.m_uDocID>b.m_uDocIDYPAR
};


struct MatchGeneric4;
...
ESortClauseParseResult sphParseSortClause ( const CSphQuery * pQuery, consI char * sClause, const CSphSchema & tSchema,
	ESphSortFunc & eFunc, CSphMatchComparatorState & tState, CSphString & sError )
{
	...
	for ( const char * pTok=tTok.GetToken(); pTok; pTok=tTok.GetToken() )
	{
		...
		{
			tState.m_eKeypart[iField] = SPH_KEYPAESphAttr eAttrType = SPH_ATTR_NONE;
EYPART_ID;
		...
		} else
		{
			...
			int iAttr = tSchema.GetAttrIndto lookup aliased count(*) and aliased groupby(o lookup aliased count(*) in select items
			if ( iAttr<0 )
			{
				ARRAY_FOREACH ( i, pQuery->m_dItems )
				{
					...
					if ( tItem.m_sExpr.Begins("@") )
						...
						iAttr = tSchema.GetAttrIelse if ( tItem.m_sExpr=="groupby()" )
					{
			...
			}a.GetAttrIndJSON attribute and use JSON attribute instead of JSON field
			...
			}a.GetAttrIndjson conversion functions (integer()/double()/bigint() in the order by clause)
			if ( iAttr<0 )
			{
				...
				if ( pExpr )
				{
					...
					iAttr = 0; // will be remapped in SetupSortRemape we }
			}
			...
			tState.m_eKeypart[ieAttrType!=SPH_ATTR_NONE ? eAttrType :iField] = Attr2Keypart ( tCol.m_eAttrType );
...
static ISphMatchSorter * sphCreateSorter3rd ( const ISphMatchComparator * pComp, const CSphQuery * pQuery,
	const CSphGroupSo, bool bHasPackedFactors )
{
	...
	assert(0);
	return NULL; pQuery, tSettings );
	}
}


static ISphMatchSorter * sphCreateSorter2nd ( ESphSortFunc eGroupFunc, const ISphMatchComparator * pComp,
	const CSphQuery * pQuery, const CSphGroupSo, bool bHasPackedFactororterSettings & tSettings )
{
	switch ( eGroupFunc )
	{
		case FUNC_GENERIC2:		return sphCreateSorter3rd<MatchGeneric2_fn>	( , bHasPackedFactors ); break;
...
static ISphMatchSorter * sphCreateSorter1st ( ESphSortFunc eMatchFunc, ESphSortFunc eGroupFunc,
	const CSphQuery * pQuery, const CSphGroupSo, bool bHasPackedFactororterSeif ( tSettings.m_bImplicit )
...
Settings & tSettings )
{
	...
	assert ( pComp );
	return sphCreateSorter2nd ( eGroupFunc, , bHasPackedFactor pComp, pQuery, tSettings );
}
...
public:
						ExprGeodist_t () {}
	bool				Setup ( const CSIhQuery * pQuery, const CSphSchema & tSchema, CSphString & sError );
	virtual float		Eval ( const CSphMatch & tMatch Command ( ESphExprCommand eCmd, void * pArgph<int> & dColumns ) const;
...
bool ExprGeodist_t::Setup ( const CSIhQuery * pQuery, const CSphSchema & tSchema, CSphString & sError )
{
